...
export default function LeaguePage(){
  ...
  async function loadDetails(id:string){
    const[orderResult,draftResult,settingsResult,windowResult,tradeResult]=await Promise.all([supabase.rpc("draft_order",{p_league_id:id}),supabase.from("drafts").select("status").eq("league_id",id).maybeSingle(),supabase.rpc("league_settings",{p_league_id:id}),supabase.rpc("transaction_window",{p_league_id:id}),supabase.rpc("unread_trade_count",{p_league_id:id})]);
    ...
    setUnreadTrades(Number(tradeResult.data??0));
  }
  ...
  return <PageShell eyebrow="PRIVATE COMPETITION" title={active&&!showMembership?active.league_name:"Your leagues"}>
    {signedIn===false?<section className="panel empty-state"><strong>Log in to create or join a league.</strong><p>Your leagues and commissioner settings are tied to your account.</p><Link className="primary-button full-button" href="/login?next=/league">Log in to continue</Link></section>:active&&!showMembership?<>
      ...
      <section className="league-command-grid">{active.game_format==="pack"?<Link href={`/packs?league=${active.league_id}`}><span>▣</span><strong>Pack club</strong><small>Collection & auction</small></Link>:<Link href={`/draft?league=${active.league_id}`}><span>◷</span><strong>Draft room</strong><small>{draft?.status??"Waiting"}</small></Link>}<Link href={`/team?league=${active.league_id}`}><span>◎</span><strong>My Team</strong><small>Lineup & captain</small></Link>{active.game_format==="pack"?<Link href={`/packs?league=${active.league_id}#auction`}><span>⌁</span><strong>Auction house</strong><small>League duplicates</small></Link>:<Link href={`/waivers?league=${active.league_id}#claims`}><span>↻</span><strong>Waivers</strong><small>Priority & claims</small></Link>}<Link className="trade-command" href={`/trades?league=${active.league_id}`}><span>⇄</span>{unreadTrades?<b className="trade-alert-count">{unreadTrades}</b>:null}<strong>Trades</strong><small>{unreadTrades?`${unreadTrades} new offer${unreadTrades===1?"":"s"}`:"Offers & history"}</small></Link></section>
