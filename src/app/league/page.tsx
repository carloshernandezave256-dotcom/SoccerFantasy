...
type Settings={league_name:string;joining_open:boolean;draft_pick_seconds:number;trades_enabled:boolean;lineup_lock_minutes:number;motm_manual:boolean;calendar_competition:string};
...
export default function LeaguePage(){
  ...
  return <PageShell eyebrow="PRIVATE COMPETITION" title={active&&!showMembership?active.league_name:"Your leagues"}>
    {signedIn===false?<section className="panel empty-state"><strong>Log in to create or join a league.</strong><p>Your leagues and commissioner settings are tied to your account.</p><Link className="primary-button full-button" href="/login?next=/league">Log in to continue</Link></section>:active&&!showMembership?<>
      ...
      {settings?<section className="panel league-member-note"><p className="eyebrow">FANTASY CALENDAR · SEASON LOCKED</p><h2>{settings.calendar_competition}</h2><p>Its official matchweeks define this league’s scoring windows and bye weeks. Other competitions count only when their matches fall inside those windows.</p></section>:null}
      ...
      {tab==="create"?<section className="panel form-card"><div className="form-section-title"><p className="eyebrow">FANTASY CALENDAR</p><strong>Choose the competition your season follows</strong></div><label>Schedule league<select value={calendarCompetition} onChange={event=>setCalendarCompetition(event.target.value)}><option>Premier League</option><option>La Liga</option><option>Serie A</option><option>Bundesliga</option><option>Ligue 1</option></select></label><p>This competition’s official matchweeks define every fantasy scoring window. Matches from the other four leagues count only when they fall inside that window. If this competition has no matchweek, everyone has a bye and nothing is calculated.</p><p className="form-message"><strong>Warning:</strong> this selection locks immediately when the league is created and cannot be changed for the season.</p></section>:null}
