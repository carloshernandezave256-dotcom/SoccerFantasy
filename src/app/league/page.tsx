...
export default function LeaguePage(){
  ...
  async function load(preferred?:string){
    ...
    const selected=resolveActiveLeague(list,preferred);const id=selected?.league_id??"";
    setLeagues(id?[...list].sort((a,b)=>Number(b.league_id===id)-Number(a.league_id===id)):list);setActiveId(id);
  ...
  function openMembership(){
    setShowMembership(true);setMessage("");
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{const{data,error}=await withTimeout(supabase.rpc(tab==="create"?"create_league":"join_league",args));if(error)setMessage(error.message);else{const id=String(data??"");setActiveLeagueId(id);formElement.reset();setCode("");if(tab==="create"){window.location.assign(`/?league=${encodeURIComponent(id)}`);return}setMessage("You joined the league.");setShowMembership(false);await load(id)}}catch(error){setMessage(error instanceof Error?error.message:"The request could not be completed.")}finally{setBusy(false)}
  }
  ...
  return <PageShell eyebrow="PRIVATE COMPETITION" title={active&&!showMembership?active.league_name:"Your leagues"}>
    {signedIn===false?<section className="panel empty-state"><strong>Log in to create or join a league.</strong><p>Your leagues and commissioner settings are tied to your account.</p><Link className="primary-button full-button" href="/login?next=/league">Log in to continue</Link></section>:active&&!showMembership?<>
      ...
      <section className="segmented league-view-tabs" aria-label="League sections"><button className={leagueView==="data"?"active":""} onClick={()=>setLeagueView("data")}>League Data</button><button className={leagueView==="settings"?"active":""} onClick={()=>setLeagueView("settings")}>Settings</button></section>
      {leagueView==="data"?<>
        <section className="panel league-overview"><div className="section-row"><div><p className="eyebrow">COMPETITION STATUS</p><h2>{active.game_format==="pack"?"Pack League":"Draft League"}</h2></div>{active.game_format==="pack"?<Link className="league-primary-link" href={`/packs?league=${active.league_id}`}>Open pack club →</Link>:draft?.status!=="complete"?<Link className="league-primary-link" href={`/draft?league=${active.league_id}`}>Open draft room →</Link>:null}</div><div className="league-data-grid"><span><small>MANAGERS</small><strong>{active.manager_count}/{active.league_size}</strong></span><span><small>{active.game_format==="pack"?"FORMAT":"DRAFT"}</small><strong>{active.game_format==="pack"?"Duplicates":"Exclusive"}</strong></span><span><small>STATUS</small><strong>{draft?.status??"Waiting"}</strong></span></div></section>
      ...
      <button className="secondary-button full-button league-secondary-action" onClick={openMembership}>Create or join another league</button>
      ...
      {tab==="create"?<section className="panel form-card"><div className="form-section-title"><p className="eyebrow">PLAYER POOL</p><strong>Choose who can be owned in this league</strong></div><label>Eligible players<select value={playerPool} onChange={event=>{const value=event.target.value;setPlayerPool(value);if(value!=="All Top Five")setCalendarCompetition(value)}}><option>All Top Five</option><option>Premier League</option><option>La Liga</option><option>Serie A</option><option>Bundesliga</option><option>Ligue 1</option></select></label><p>All Top Five is the signature cross-league experience. Choosing one competition limits the draft, packs, waivers and trades to players from that league.</p><div className="form-section-title"><p className="eyebrow">FANTASY CALENDAR</p><strong>Choose the competition your season follows</strong></div><label>Schedule league<select value={playerPool==="All Top Five"?calendarCompetition:playerPool} disabled={playerPool!=="All Top Five"} onChange={event=>setCalendarCompetition(event.target.value)}><option>Premier League</option><option>La Liga</option><option>Serie A</option><option>Bundesliga</option><option>Ligue 1</option></select></label><div className="creation-rule-summary"><strong>How your fantasy calendar works</strong><span>{playerPool==="All Top Five"?`${calendarCompetition} matchweeks become this fantasy league’s official matchweeks.`:`${playerPool} matchweeks automatically become this fantasy league’s official matchweeks.`}</span><span>Your weekly score is the total earned by your locked Starting XI during that official matchweek’s fixtures.</span><span>{playerPool==="All Top Five"?`Players from the other supported leagues count only when their fixtures fall inside the ${calendarCompetition} matchweek window.`:`Only ${playerPool} players are eligible and score.`}</span><span>If the selected competition has no matchweek, every manager has a bye and no matchup is calculated.</span></div><p className="form-message"><strong>Season lock:</strong> the player pool and fantasy calendar cannot be changed after the league is created.</p></section>:null}
