...
type PriorityDrag = { kind:"claim"|"offer";index:number;target:number;top:number;left:number;width:number;height:number;pointerOffsetY:number;label:string };
...
export default function WaiversPage() {
  ...
  const priorityDragRef=useRef<PriorityDrag|null>(null);
  const priorityListRef=useRef<HTMLElement|null>(null);
  ...
  async function submitClaim() {
    ...
    const rpc=isAuction?(windowState?.phase==="free_agency"?"sign_available_player":"submit_contract_offer"):(windowState?.phase==="free_agency"?"pickup_free_agent":"submit_waiver_claim");
    const params=isAuction?(windowState?.phase==="free_agency"?{p_league_id:league.league_id,p_add_player_id:selected.id,p_release_player_id:Number(dropId)}:{p_league_id:league.league_id,p_add_player_id:selected.id,p_release_player_id:Number(dropId),p_amount:amount}):{p_league_id:league.league_id,p_add_player_id:selected.id,p_drop_player_id:Number(dropId)};
    const { error } = await supabase.rpc(rpc, params);
    if (error) setMessage(error.message); else { setMessage(isAuction?(windowState?.phase==="free_agency"?`${selected.full_name} signed a $1M contract.`:`Blind contract offer submitted for ${selected.full_name}.`):(windowState?.phase==="free_agency"?`${selected.full_name} was added to your roster.`:`Claim submitted for ${selected.full_name}.`)); setSelected(null); setDropId("");setOfferAmount(""); await loadLeague(league, userId); }
    setBusy(false);
  }
  ...
  async function savePriorityOrder(kind:"claim"|"offer",index:number,target:number){
    if(!league||index===target)return;
    const pending=kind==="claim"
      ?claims.filter(claim=>claim.user_id===userId&&claim.status==="pending").sort((a,b)=>a.claim_rank-b.claim_rank)
      :contractOffers.filter(offer=>offer.user_id===userId&&offer.status==="pending").sort((a,b)=>a.offer_rank-b.offer_rank);
    const ids=pending.map(item=>item.id),[moved]=ids.splice(index,1);ids.splice(target,0,moved);
    setBusy(true);setMessage("");
    const{error}=await supabase.rpc(kind==="claim"?"reorder_waiver_claims":"reorder_contract_offers",kind==="claim"?{p_league_id:league.league_id,p_claim_ids:ids}:{p_league_id:league.league_id,p_offer_ids:ids});
    if(error)setMessage(error.message);else{setMessage(`${kind==="claim"?"Claim":"Offer"} priority updated.`);await loadLeague(league,userId)}
    setBusy(false);
  }

  function beginPriorityDrag(event:React.PointerEvent<HTMLButtonElement>,kind:"claim"|"offer",index:number,label:string){
    if(busy)return;const row=event.currentTarget.closest<HTMLElement>("[data-priority-index]");if(!row)return;
    event.preventDefault();event.currentTarget.setPointerCapture(event.pointerId);const rect=row.getBoundingClientRect();
    const drag={kind,index,target:index,top:rect.top,left:rect.left,width:rect.width,height:rect.height,pointerOffsetY:event.clientY-rect.top,label};
    priorityDragRef.current=drag;setPriorityDrag(drag);
  }

  function updatePriorityDrag(event:React.PointerEvent<HTMLButtonElement>){
    const active=priorityDragRef.current;if(!active)return;event.preventDefault();
    const rows=Array.from(priorityListRef.current?.querySelectorAll<HTMLElement>(`[data-priority-kind="${active.kind}"]`)??[]);let target=active.index;
    for(const row of rows){const index=Number(row.dataset.priorityIndex);if(index===active.index)continue;const rect=row.getBoundingClientRect();if(event.clientY>rect.top+rect.height/2)target=index;else{target=index;break}}
    if(rows.length&&event.clientY<rows[0].getBoundingClientRect().top)target=0;
    if(rows.length&&event.clientY>rows[rows.length-1].getBoundingClientRect().bottom)target=rows.length-1;
    const next={...active,target,top:event.clientY-active.pointerOffsetY};priorityDragRef.current=next;setPriorityDrag(next);
    if(event.clientY<72)window.scrollBy({top:-12,behavior:"auto"});else if(event.clientY>window.innerHeight-72)window.scrollBy({top:12,behavior:"auto"});
  }

  function finishPriorityDrag(event:React.PointerEvent<HTMLButtonElement>){
    const active=priorityDragRef.current;if(!active)return;
    if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);
    priorityDragRef.current=null;setPriorityDrag(null);void savePriorityOrder(active.kind,active.index,active.target);
  }

  const sortedContractOffers=[...contractOffers].sort((a,b)=>a.status==="pending"&&b.status==="pending"?a.offer_rank-b.offer_rank:new Date(b.created_at).getTime()-new Date(a.created_at).getTime());
  const sortedClaims=[...claims].sort((a,b)=>a.status==="pending"&&b.status==="pending"?a.claim_rank-b.claim_rank:new Date(b.created_at).getTime()-new Date(a.created_at).getTime());
  ...
  return <PageShell eyebrow={isAuction?"AUCTION CONTRACTS":"ROSTER MOVES"} title={isAuction?"Contract market":"Player market"}>
    ...
    {!userId && !loading ? <section className="panel empty-feature"><span>↻</span><h2>Sign in to manage waivers</h2><p>Your claims, roster, and waiver priority are private to your league.</p><Link className="primary-button" href="/login?next=/waivers">Log in</Link></section> : !league && !loading ? <section className="panel empty-feature"><span>＋</span><h2>Join a league first</h2><p>Waivers open for managers after a league draft is complete.</p><Link className="primary-button" href="/league">Open leagues</Link></section> : <>
      ...
      {tab === "market" ? <section className="panel">
        ...
        <div className="player-list waiver-player-list">{marketPlayers.slice(0, 60).map((player) => { const owner = ownerByPlayer.get(player.id); const totals = totalsByPlayer.get(player.id) ?? emptyTotals; return <article key={player.id} role="button" tabIndex={0} onClick={() => setStatsPlayer(player)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setStatsPlayer(player); }}><PlayerHeadshot name={player.full_name} position={player.position} photoUrl={player.photo_url}/><div><strong>{player.full_name}</strong><small>#{player.draft_rank ?? "—"} · {player.club} · {player.competition}</small>{owner ? <small className="ownership-label">{isAuction?"Under contract with":"Owned by"} {owner}</small> : <small className="available-label">Available · tap for stats</small>}</div><b className="fantasy-points-chip">{totals.points} FP</b>{owner ? <span className="owned-chip">{isAuction?"SIGNED":"OWNED"}</span> : <button className="claim-button" disabled={!windowState||windowState.phase==="locked"} onClick={(event) => { event.stopPropagation(); setSelected(player); setDropId("");setOfferAmount(""); }}>{isAuction?(windowState?.phase==="free_agency"?"SIGN $1M":"OFFER"):windowState?.phase==="free_agency"?"ADD NOW":"CLAIM"}</button>}</article>; })}{!loading && marketPlayers.length === 0 ? <p className="empty-state">No players match those filters.</p> : null}</div>
      </section> : isAuction?<section ref={priorityListRef} className={`panel claim-history priority-reorder-list ${priorityDrag?.kind==="offer"?"is-reordering":""}`}><div className="section-row"><div><h2>My contract offers</h2><small>Drag the handle to set private-offer priority</small></div><span className="muted-chip">{contractOffers.length}</span></div>{contractOffers.length===0?<p className="empty-state">No contract offers yet.</p>:sortedContractOffers.map(offer=>{const add=playerMap.get(offer.add_player_id),release=playerMap.get(offer.release_player_id),pending=offer.user_id===userId&&offer.status==="pending",reorderIndex=pending?sortedContractOffers.filter(item=>item.user_id===userId&&item.status==="pending").findIndex(item=>item.id===offer.id):-1,label=`${add?.full_name??"Player"} · $${Math.round(offer.amount/1000000)}M`;return <article data-priority-index={pending?reorderIndex:undefined} data-priority-kind={pending?"offer":undefined} className={`${priorityDrag?.kind==="offer"&&priorityDrag.index===reorderIndex?"priority-drag-source":""} ${priorityDrag?.kind==="offer"&&priorityDrag.target===reorderIndex&&priorityDrag.index!==reorderIndex?(priorityDrag.index<reorderIndex?"priority-drop-after":"priority-drop-before"):""}`} key={offer.id}>{pending?<button className="priority-drag-handle" disabled={busy} onPointerDown={event=>beginPriorityDrag(event,"offer",reorderIndex,label)} onPointerMove={updatePriorityDrag} onPointerUp={finishPriorityDrag} onPointerCancel={finishPriorityDrag} aria-label={`Reorder offer for ${add?.full_name??"player"}`}>☰</button>:null}<div className="claim-details"><span className={`claim-status ${offer.status}`}>{pending?`#${offer.offer_rank} private`:offer.status}</span><strong>{label}</strong><small>GW {offer.gameweek} · Release {release?.full_name??"player"}{offer.note?` · ${offer.note}`:""}</small></div>{pending?<button className="claim-cancel" onClick={()=>void cancelContractOffer(offer.id)} disabled={busy}>Cancel</button>:null}</article>})}{priorityDrag?.kind==="offer"?<div className="priority-drag-ghost" aria-hidden="true" style={{top:priorityDrag.top,left:priorityDrag.left,width:priorityDrag.width,height:priorityDrag.height}}><span>☰</span><strong>{priorityDrag.label}</strong><b>#{priorityDrag.target+1}</b></div>:null}</section>:<><section className="panel waiver-priority"><div className="section-row"><h2>Waiver priority</h2><small>Randomized each gameweek</small></div>{priority.map((item) => <article key={item.user_id} className={item.user_id === userId ? "you" : ""}><span>{item.rank}</span><strong>{item.team_name}</strong>{item.user_id === userId ? <small>YOU</small> : null}</article>)}</section><section ref={priorityListRef} className={`panel claim-history priority-reorder-list ${priorityDrag?.kind==="claim"?"is-reordering":""}`}><div className="section-row"><div><h2>Claim history</h2><small>Drag pending claims to change priority</small></div><span className="muted-chip">{claims.length}</span></div>{claims.length === 0 ? <p className="empty-state">No waiver claims yet.</p> : sortedClaims.map((claim) => { const add = playerMap.get(claim.add_player_id); const drop = claim.drop_player_id ? playerMap.get(claim.drop_player_id) : null; const minePending=claim.user_id===userId&&claim.status==="pending",reorderIndex=minePending?sortedClaims.filter(item=>item.user_id===userId&&item.status==="pending").findIndex(item=>item.id===claim.id):-1,label=`${teamMap.get(claim.user_id) ?? "Your team"}: ${add?.full_name ?? "Player"}`;return <article data-priority-index={minePending?reorderIndex:undefined} data-priority-kind={minePending?"claim":undefined} className={`${priorityDrag?.kind==="claim"&&priorityDrag.index===reorderIndex?"priority-drag-source":""} ${priorityDrag?.kind==="claim"&&priorityDrag.target===reorderIndex&&priorityDrag.index!==reorderIndex?(priorityDrag.index<reorderIndex?"priority-drop-after":"priority-drop-before"):""}`} key={claim.id}>{minePending?<button className="priority-drag-handle" disabled={busy} onPointerDown={event=>beginPriorityDrag(event,"claim",reorderIndex,label)} onPointerMove={updatePriorityDrag} onPointerUp={finishPriorityDrag} onPointerCancel={finishPriorityDrag} aria-label={`Reorder claim for ${add?.full_name??"player"}`}>☰</button>:null}<div className="claim-details"><span className={`claim-status ${claim.status}`}>{minePending?`#${claim.claim_rank} ${claim.status}`:claim.status}</span><strong>{label}</strong><small>GW {claim.gameweek} · {drop ? `Drop ${drop.full_name} · ` : ""}{new Date(claim.created_at).toLocaleString()}{claim.note ? ` · ${claim.note}` : ""}</small></div>{minePending?<button className="claim-cancel" onClick={() => void cancelClaim(claim.id)} disabled={busy}>Cancel</button>:null}</article>; })}{priorityDrag?.kind==="claim"?<div className="priority-drag-ghost" aria-hidden="true" style={{top:priorityDrag.top,left:priorityDrag.left,width:priorityDrag.width,height:priorityDrag.height}}><span>☰</span><strong>{priorityDrag.label}</strong><b>#{priorityDrag.target+1}</b></div>:null}</section></>}
    </>}
    {selected ? <div className="waiver-overlay" role="dialog" aria-modal="true" aria-label="Submit player transaction"><section className="panel waiver-dialog"><p className="eyebrow">{isAuction?(windowState?.phase==="free_agency"?"$1M AVAILABLE CONTRACT":"BLIND CONTRACT OFFER"):windowState?.phase==="free_agency"?"FREE AGENT":"WAIVER CLAIM"}</p><h2>{isAuction?"Sign":"Add"} {selected.full_name}</h2><p>{isAuction?"Your offer stays private until the window closes. Choose the player whose current contract will be released.":"Select the player who will leave your 18-player roster. Position limits and four players per club are enforced."}</p>{isAuction&&windowState?.phase!=="free_agency"?<label>Contract offer ($M)<input type="number" min="1" step="1" inputMode="numeric" value={offerAmount} onChange={event=>setOfferAmount(event.target.value)} placeholder="Minimum $1M"/><small>${Math.round(contractBudget/1000000)}M available</small></label>:null}<label>{isAuction?"Release from your roster":"Drop from your roster"}<select value={dropId} onChange={(event) => setDropId(event.target.value)}><option value="">Choose a player</option>{roster.map((player) => <option key={player.id} value={player.id}>{player.position} · {player.full_name}</option>)}</select></label><div><button className="secondary-button" onClick={() => setSelected(null)} disabled={busy}>Back</button><button className="primary-button" onClick={() => void submitClaim()} disabled={busy || !dropId||(isAuction&&windowState?.phase!=="free_agency"&&(!offerAmount||Number(offerAmount)<1||Number(offerAmount)*1000000>contractBudget))}>{busy ? "Saving…" : isAuction?(windowState?.phase==="free_agency"?"Sign for $1M":"Submit private offer"):windowState?.phase==="free_agency"?"Add now":"Submit claim"}</button></div></section></div> : null}
