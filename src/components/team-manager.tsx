...
export function TeamManager(){
  ...
  function moveStarter(id:number,direction:-1|1){
    const position=roster.find(player=>player.id===id)?.position;
    if(!position)return;
    setStarterOrder(order=>{
      const peers=order.filter(playerId=>starters.has(playerId)&&roster.find(player=>player.id===playerId)?.position===position);
      const index=peers.indexOf(id),swap=peers[index+direction];
      if(index<0||!swap)return order;
      return order.map(playerId=>playerId===id?swap:playerId===swap?id:playerId);
    });
  }
  ...
  return <PageShell eyebrow={viewedManager?.team_name??leagues.find(item=>item.league_id===league)?.team_name??"MY CLUB"} title={isMine?"My Team":"Team Viewer"}>
    ...
    {loading?<section className="panel empty-state">Loading squad…</section>:roster.length===0?(isMine?<TeamDemo/>:<section className="panel empty-state"><strong>{viewedManager?.team_name} has no players yet.</strong><p>Their drafted squad will appear here as picks are made.</p></section>):<>
      ...
      {isMine&&editing?<><div className="team-controls"><button className={captainMode?"active":""} onClick={()=>{setCaptainMode(active=>!active);setSelectedBench(null);setMessage("Captain mode: tap one of your starters on the pitch.")}}>© Set captain</button><span className={valid?"valid":"invalid"}>{valid?"✓ Ready to save":"! Choose a captain"}</span></div><p className="team-instruction">{message||"Tap a bench player to make a substitution, or use the arrows to arrange each row."}</p><SavedTeamPitch roster={roster} starters={starters} starterOrder={starterOrder} captain={captain} editing captainMode={captainMode} selectedBench={selectedBench} onStarter={tapStarter} onMove={moveStarter} onBench={id=>{setCaptainMode(false);setSelectedBench(id);setMessage(`${roster.find(player=>player.id===id)?.full_name} selected. Now tap a starter on the pitch.`)}}/><button className="primary-button full-button" disabled={!valid} onClick={save}>Save lineup</button></>:<><SavedTeamPitch roster={roster} starters={starters} starterOrder={starterOrder} captain={captain}/>{isMine?<button className="primary-button full-button edit-lineup-button" onClick={()=>{setEditing(true);setSelectedBench(null);setMessage("Tap a bench player to make a substitution, or use the arrows to arrange each row.")}}>Edit lineup</button>:<div className="view-only-banner">Viewing {viewedManager?.team_name} · Read only</div>}</>}
    </>}
    {message&&isMine?<p className="form-message">{message}</p>:null}
  </PageShell>;
}

function SavedTeamPitch({roster,starters,starterOrder,captain,editing=false,captainMode=false,selectedBench=null,onStarter,onBench,onMove}:{roster:Player[];starters:Set<number>;starterOrder:number[];captain:number|null;editing?:boolean;captainMode?:boolean;selectedBench?:number|null;onStarter?:(id:number)=>void;onBench?:(id:number)=>void;onMove?:(id:number,direction:-1|1)=>void}){
