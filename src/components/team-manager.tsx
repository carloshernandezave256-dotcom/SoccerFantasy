...
export function TeamManager(){
  ...
  function tapStarter(id:number){
    ...
    if(captainMode){
      const previous=captain;setCaptain(id);setDirty(true);setCaptainMode(false);setSelectedBench(null);setSelectedStarter(null);void persistCaptain(id,previous);return;
    }
    if(selectedBench===null){
      if(selectedStarter===null){setSelectedStarter(id);setMessage(`${roster.find(player=>player.id===id)?.full_name} selected. Tap another ${roster.find(player=>player.id===id)?.position} to swap their positions.`);return}
      if(selectedStarter===id){setSelectedStarter(null);setMessage("Player position selection cleared.");return}
  ...
  async function persistCaptain(nextCaptain:number,rollback:number|null){
    ...
    else{setDirty(false);setMessage(`${roster.find(player=>player.id===nextCaptain)?.full_name} is saved as your Star Pick.`)}
  }
  ...
  function resetTo433(){
    ...
    setStarters(next);setStarterOrder(order);if(captain!==null&&!next.has(captain))setCaptain(null);setEditing(true);setSelectedBench(null);setSelectedStarter(null);setCaptainMode(false);setDirty(true);setMessage("Reset to the default 4-3-3. Save to keep this lineup.");
  }
  ...
  async function save(){
    if(lineupLocked){setMessage("Your full lineup is locked until every fixture in this gameweek is final.");return}
    if(!isMine||!valid||captain===null){setMessage("Choose a valid starting XI and Star Pick before saving.");return}
    suppressRefreshUntil.current=Date.now()+4000;
  ...
  return <PageShell leagueId={league} eyebrow={viewedManager?.team_name??leagues.find(item=>item.league_id===league)?.team_name??"MY CLUB"} title={isMine?"My Team":"Team Viewer"}>
    {draftComplete&&isMine?<section className="panel draft-finished"><span>✓</span><h2>Draft complete — set your lineup</h2><p>Your 4-3-3 has been created. Choose the starter you predict will score the most points as your Star Pick, make any changes, and save.</p></section>:null}
    ...
    {loading?<section className="panel empty-state">Loading squad…</section>:roster.length===0?(showPackCards&&isMine?<section className="panel empty-state"><strong>Your pack squad is waiting.</strong><p>Open your starter bundle first. Every packed player will save to this league, and the first 18 unique cards will refresh into My Team automatically.</p><Link className="primary-button full-button" href={`/packs?league=${league}`}>Open starter bundle</Link></section>:isMine?<section className="panel empty-state"><strong>Waiting for this league&apos;s draft.</strong><p>Your players will appear here as this league drafts. No roster from another league will be shown.</p></section>:<section className="panel empty-state"><strong>{viewedManager?.team_name} has no players yet.</strong><p>{showPackCards?"Their packed players will appear here after they open a starter bundle.":"Their drafted squad will appear here as picks are made."}</p></section>):<>
      ...
      {isMine&&lineupLocked?<section className="panel lineup-lock-banner"><strong>Gameweek {lineupLock?.gameweek} lineup locked</strong><p>Your starting XI, bench order, player arrangement, and Star Pick reopen together after every fixture in this gameweek is final.</p></section>:null}
      {isMine&&editing?<><div className="team-controls"><button type="button" className={captainMode?"active":""} onClick={()=>{setCaptainMode(active=>!active);setSelectedBench(null);setSelectedStarter(null);setMessage("Star Pick mode: tap the starter you predict will score the most points this gameweek.")}}>★ Tap Star Pick</button><select className="league-select captain-select" aria-label="Choose Star Pick" value={captain??""} onChange={event=>{const next=Number(event.target.value);if(!next)return;const previous=captain;setCaptain(next);setCaptainMode(false);setSelectedStarter(null);setDirty(true);void persistCaptain(next,previous)}}><option value="">Choose Star Pick</option>{starterOrder.filter(id=>starters.has(id)).flatMap(id=>{const player=roster.find(item=>item.id===id);return player?[<option key={player.id} value={player.id}>{player.full_name}</option>]:[]})}</select><span className={valid?"valid":"invalid"}>{dirty?"● Saving arrangement":valid?"✓ Lineup saved":"Arrange now · Star Pick when ready"}</span></div><p className="team-instruction">{message||"Tap two players in the same position row to swap them, or press and drag."}</p><SavedTeamPitch roster={roster} starters={starters} starterOrder={starterOrder} captain={captain} showPackCards={showPackCards} editing captainMode={captainMode} selectedBench={selectedBench} selectedStarter={selectedStarter} onInfo={id=>setInfoPlayer(roster.find(player=>player.id===id)??null)} onStarter={tapStarter} onReorder={(id,targetId)=>{setSelectedStarter(null);reorderStarter(id,targetId)}} onBench={id=>{setCaptainMode(false);setSelectedStarter(null);setSelectedBench(id);setMessage(`${roster.find(player=>player.id===id)?.full_name} selected. Now tap a starter on the pitch.`)}}/><div className="lineup-edit-actions"><button type="button" className="secondary-button" onClick={resetTo433}>Reset to 4-3-3</button><button type="button" className="secondary-button" disabled={!dirty&&!undoOrder} onClick={()=>void undoChanges()}>Undo changes</button></div><button type="button" className="primary-button full-button" disabled={!valid} onClick={save}>{captain===null?"Choose a Star Pick to finish":"Finish lineup"}</button></>:<><SavedTeamPitch roster={roster} starters={starters} starterOrder={starterOrder} captain={captain} showPackCards={showPackCards} allowDrag={isMine} onReorder={(id,targetId)=>{setEditing(true);reorderStarter(id,targetId)}} onInfo={id=>setInfoPlayer(roster.find(player=>player.id===id)??null)} onBench={id=>setInfoPlayer(roster.find(player=>player.id===id)??null)}/>{isMine?<button className="primary-button full-button edit-lineup-button" onClick={()=>{setEditing(true);setSelectedBench(null);setSelectedStarter(null);setMessage("Tap two players in the same position row to swap them, or press and drag.")}}>Edit lineup</button>:<div className="view-only-banner">Viewing {viewedManager?.team_name} · Read only</div>}</>}
    </>}
...
function SavedTeamPitch({roster,starters,starterOrder,captain,editing=false,allowDrag=editing,captainMode=false,selectedBench=null,selectedStarter=null,onStarter,onBench,onReorder,onInfo,showPackCards=false}:{roster:Player[];starters:Set<number>;starterOrder:number[];captain:number|null;editing?:boolean;allowDrag?:boolean;captainMode?:boolean;selectedBench?:number|null;selectedStarter?:number|null;onStarter?:(id:number)=>void;onBench?:(id:number)=>void;onReorder?:(id:number,targetId:number)=>void;onInfo?:(id:number)=>void;showPackCards?:boolean}){
  ...
  function updatePitchDrag(event:React.PointerEvent<HTMLButtonElement>){
    ...
    const moved=allowDrag&&!captainMode&&(active.moved||Math.hypot(event.clientX-(active.left+active.offsetX),event.clientY-(active.top+active.offsetY))>7);
...
function PitchPlayer({player,editing,captain,selectedStarter,showPackCards,pitchDrag,onPointerDown,onPointerMove,onPointerUp}:{player:Player;editing:boolean;captain:number|null;selectedStarter:number|null;showPackCards:boolean;pitchDrag:PitchDrag|null;onPointerDown:(event:React.PointerEvent<HTMLButtonElement>,player:Player)=>void;onPointerMove:(event:React.PointerEvent<HTMLButtonElement>)=>void;onPointerUp:(event:React.PointerEvent<HTMLButtonElement>)=>void}){
 ...
 return <div className={`pitch-player-slot ${selectedStarter===player.id?"pitch-tap-selected":""} ${pitchDrag?.moved&&pitchDrag.id===player.id?"pitch-drag-source":""} ${pitchDrag?.moved&&pitchDrag.targetId===player.id&&pitchDrag.id!==player.id?"pitch-drop-target":""}`} data-position={player.position} data-player-id={player.id}><button type="button" className="saved-pitch-player" onPointerDown={event=>onPointerDown(event,player)} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onClick={event=>event.preventDefault()} aria-disabled={!editing}><span className={`shirt shirt-${player.position.toLowerCase()} ${hero?"mini-card-shirt":""}`}>{hero?<img src={hero.src} alt=""/>:player.photo_url?<img className="api-headshot" src={player.photo_url} alt="" onError={event=>{event.currentTarget.style.display="none"}}/>:null}{player.injured?<span className="injury-cross" title={player.injury_reason??player.injury_type??"Unavailable"}>✚</span>:null}{captain===player.id?<b title="Star Pick">★</b>:null}</span><strong>{player.full_name}</strong><small>{player.club}</small></button></div>
}
