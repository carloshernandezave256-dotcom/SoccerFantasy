...
export function TeamManager(){
  ...
  useEffect(()=>{
    void(async()=>{
      ...
      setLeagues(list);
      if(list[0]){setLeague(list[0].league_id);const{data:order}=await supabase.rpc("draft_order",{p_league_id:list[0].league_id});const managersList=(order??[]) as Manager[];setManagers(managersList);const owner=managersList.some(manager=>manager.user_id===user.id)?user.id:managersList[0]?.user_id??"";setViewedUser(owner);if(owner)await loadRoster(list[0].league_id,owner)}
