...
export function RealMatchup(){
  ...
  useEffect(()=>{
    ...
    void(async()=>{
      ...
      const[lineupResult,picksResult,scoreResult,matchupResult,standingsResult]=await Promise.all([
        supabase.from("lineup_players").select("user_id,is_starter,is_captain,players(id,full_name,position,club)").eq("league_id",league.league_id).in("user_id",ids),
        supabase.from("draft_picks").select("user_id,pick_number,players(id,full_name,position,club)").eq("league_id",league.league_id).in("user_id",ids).order("pick_number"),
        supabase.from("league_player_scores").select("player_id,minutes,goals,assists,shots_on_target,big_chances_missed,completed_passes,tackles_won,penalty_goals,penalties_missed,penalties_conceded,saves,penalties_saved,goals_conceded,yellow_cards,second_yellow_cards,red_cards,own_goals,man_of_the_match,status").eq("league_id",league.league_id).eq("gameweek",gameweek),
      ...
      const lineupRows=(lineupResult.data??[]) as unknown as {user_id:string;is_starter:boolean;is_captain:boolean;players:Omit<Player,"score"|"status"|"ledger">|null}[];
      const pickRows=(picksResult.data??[]) as unknown as {user_id:string;pick_number:number;players:Omit<Player,"score"|"status"|"ledger">|null}[];
      ...
      const build=(owner:string,score:number|string):TeamView=>{
        const saved=lineupRows.filter(row=>row.user_id===owner&&row.is_starter&&row.players);
