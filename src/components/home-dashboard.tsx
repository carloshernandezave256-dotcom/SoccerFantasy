...
export function HomeDashboard(){
  ...
  const load=useCallback(async()=>{
    ...
    const{data:leagues}=await supabase.rpc("my_leagues");
    const active=((leagues??[]) as League[])[0]??null;
    setLeague(active);
  ...
  return <main className="app-shell home-dashboard">
    ...
    {league?<>
      <section className="home-news" aria-labelledby="player-news-title">
        ...
        <div className="news-scroll">{newsPreview.map(item=><article key={item.tag} className={item.parody?"breaking-parody":""} onClick={item.parody?()=>setParodyOpen(true):undefined} role={item.parody?"button":undefined} tabIndex={item.parody?0:undefined} onKeyDown={item.parody?event=>{if(event.key==="Enter"||event.key===" ")setParodyOpen(true)}:undefined}>{item.parody?<Image className="breaking-news-image" src="/news/haaland-barcelona-parody.webp" alt="Parody transfer graphic of Erling Haaland wearing Barcelona colors" width={768} height={512}/>:null}<div className="news-card-top"><span className="news-icon">{item.icon}</span><b>{item.tag}</b></div><strong>{item.title}</strong><p>{item.copy}</p><small>{item.parody?"XI TRANSFER DESK · TAP FOR SOURCE":"SAMPLE · LIVE SOURCE COMING NEXT"}</small></article>)}</div>
      </section>
      <section className="quick-grid home-actions" aria-label="League shortcuts"><Link href="/team"><span className="icon">◎</span><strong>My Team</strong><small>{myPicks.length}/18 drafted</small></Link><Link href="/players"><span className="icon">⌕</span><strong>Players</strong><small>Search five leagues</small></Link><Link href="/trades"><span className="icon">⇄</span><strong>Trades</strong><small>Build an offer</small></Link></section>
