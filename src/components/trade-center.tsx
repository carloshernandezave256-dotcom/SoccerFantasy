...
type Player={id:number;full_name:string;position:string;club:string;photo_url?:string|null};
...
export function TradeCenter(){
  ...
  const loadLeague=useCallback(async(id:string,currentUser:string)=>{
    ...
    const[orderResult,pickResult,tradeResult]=await Promise.all([
      ...
      supabase.from("trades").select("id,league_id,proposer_id,recipient_id,status,note,created_at,responded_at,trade_players(player_id,from_user_id,players(id,full_name,position,club,photo_url))").eq("league_id",id).order("created_at",{ascending:false}),
    ]);
