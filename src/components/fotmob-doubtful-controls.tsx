...
export function FotmobDoubtfulControls(){
 ...
 return <section className="panel settings-form">
  <div className="section-row"><div><p className="eyebrow">FOTMOB · SECONDARY CHECK</p><h2>Doubtful players</h2></div><span className="muted-chip">READ ONLY</span></div>
  <p>Check all five leagues for players explicitly listed as Doubtful. Review the list here; this does not change player cards or SportMonks injury records.</p>
  <button className="secondary-button full-button" type="button" disabled={busy} onClick={()=>void refresh()}>{busy?progress:'Check FotMob doubtful players'}</button>
  ...
  {reports.map(report=><details key={report.league} open><summary><strong>{report.league} · {report.players.length}</strong></summary><p className="settings-note">Checked {new Date(report.checkedAt).toLocaleString()} · {report.clubsChecked}/{report.clubsExpected} clubs</p>{report.players.length?<ul style={{listStyle:'none',padding:0}}>{report.players.map(player=><li key={player.id} style={{padding:'12px 0',borderTop:'1px solid var(--line)'}}><div className="section-row"><strong>{player.name}</strong><span style={{color:'#fbbf24',background:'#78350f55',border:'1px solid #b45309',borderRadius:999,padding:'4px 8px',fontSize:12,whiteSpace:'nowrap'}}> ? Doubtful</span></div><small>{player.club} · <a href={player.sourceUrl} target="_blank" rel="noopener noreferrer">Source: FotMob</a></small></li>)}</ul>:<p>{report.errors.length?'No doubtful players found in the clubs checked.':'No players listed as doubtful.'}</p>}</details>)}
 </section>;
}
