...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_players) loop
    ...
    if v_name is not null then
      update public.players set full_name=v_name
      where api_football_id=(v_item->>'apiFootballId')::bigint and full_name is distinct from v_name;
...
$$;
revoke all on function public.sync_api_player_common_names(jsonb) from public,anon,authenticated;
grant execute on function public.sync_api_player_common_names(jsonb) to service_role;
