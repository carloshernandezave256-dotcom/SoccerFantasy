...
comment on function public.finalize_api_football_draft_pool(jsonb) is 'Ranks players returned by the latest API-Football squad sync while preserving previously active players that may be temporarily omitted by one provider response.';
