  ...
  for v_item in select value from jsonb_array_elements(p_players) loop
    ...
    if v_id is null then
      ...
      set provider_id = v_provider_id,
          ...
          club = coalesce(nullif(btrim(v_item->>'club'), ''), club),
          competition = coalesce(nullif(btrim(v_item->>'competition'), ''), competition),
...
grant execute on function public.sync_api_football_players(jsonb)
  to service_role;

comment on function public.sync_api_football_players(jsonb) is
  'Uses the latest API-Football squad as the current club source while preserving each existing fantasy position. Later match appearances may reconcile the club again.';
