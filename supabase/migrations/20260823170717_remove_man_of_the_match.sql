-- Man of the Match is removed from fantasy scoring and display.
-- The legacy input remains in the function signature for database compatibility.
...
begin
  ...
  return jsonb_build_object('version','2026-08-23-v3','total',v_total,'entries',v_entries);
end $$;

alter table public.football_fixture_player_stats alter column calculator_version set default '2026-08-23-v3';
alter table public.league_player_scores alter column calculator_version set default '2026-08-23-v3';

-- Clear every historical/provider award before recalculating all stored scores.
update public.football_fixture_player_stats set man_of_the_match=false;
update public.league_player_scores set man_of_the_match=false;

-- Fire the existing canonical row triggers so every stored fixture and league
-- score is recalculated immediately without any MOTM points.
