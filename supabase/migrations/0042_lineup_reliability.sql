...
begin
  ...
  select l.game_format into v_format
  from public.league_members m join public.leagues l on l.id=m.league_id
  where m.league_id=p_league_id and m.user_id=v_user;
  if v_format is null then raise exception 'League membership required'; end if;

  select count(*) filter(where lp.is_starter),
    count(*) filter(where lp.is_starter and p.position='GK'),
    count(*) filter(where lp.is_starter and p.position='DEF'),
    count(*) filter(where lp.is_starter and p.position='MID'),
    count(*) filter(where lp.is_starter and p.position='FWD')
  into v_existing,v_gk,v_def,v_mid,v_fwd
  from public.lineup_players lp join public.players p on p.id=lp.player_id
  where lp.league_id=p_league_id and lp.user_id=v_user;
  if v_existing=11 and v_gk=1 and v_def>=3 and v_mid>=1 and v_fwd>=1 then return false; end if;
  ...
  select count(*) filter(where position='GK'),count(*) filter(where position='DEF'),count(*) filter(where position='MID'),count(*) filter(where position='FWD')
  into v_gk,v_def,v_mid,v_fwd from public.players where id=any(p_starters);
  ...
  if v_format='pack' then
    select count(distinct player_id) into v_owned from public.pack_cards
    where league_id=p_league_id and user_id=v_user and active_slot is not null and player_id=any(p_starters||p_bench);
  else
    select count(*) into v_owned from public.draft_picks
    where league_id=p_league_id and user_id=v_user and player_id=any(p_starters||p_bench);
  end if;
  ...
  insert into public.lineup_players(league_id,user_id,player_id,is_starter,is_captain,pitch_order)
    select p_league_id,v_user,x,true,false,ord from unnest(p_starters) with ordinality s(x,ord);
  insert into public.lineup_players(league_id,user_id,player_id,is_starter,is_captain,bench_order)
    select p_league_id,v_user,x,false,false,ord from unnest(p_bench) with ordinality b(x,ord);
  return true;
end$$;
...
revoke all on function public.initialize_default_lineup(uuid,bigint[],bigint[]),public.save_lineup(uuid,bigint[],bigint[],bigint) from public,anon;
grant execute on function public.initialize_default_lineup(uuid,bigint[],bigint[]),public.save_lineup(uuid,bigint[],bigint[],bigint) to authenticated;
