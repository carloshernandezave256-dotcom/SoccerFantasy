-- La Liga beta starter bundles use their own guaranteed hero cards while
-- every remaining card continues to obey the league's locked player pool.
...
begin
  ...
  if v_pool='La Liga' then
    select id into v_id from public.players where active and competition='La Liga' and full_name='Kylian Mbappé' limit 1;
  elsif v_pool='All Top Five' then
    select id into v_id from public.players where active and full_name='Virgil van Dijk' limit 1;
  else
    select id into v_id from public.players where active and competition=v_pool and is_club_captain order by random() limit 1;
  end if;
  ...
  if v_pool='La Liga' then
    select id into v_id from public.players where active and competition='La Liga' and full_name='Lamine Yamal' and not(id=any(v_ids)) limit 1;
  elsif v_pool='All Top Five' then
    select id into v_id from public.players where active and full_name='Erling Haaland' and not(id=any(v_ids)) limit 1;
  else
    select id into v_id from public.players where active and competition=v_pool and draft_rank<=50 and not(id=any(v_ids)) order by random() limit 1;
  end if;
