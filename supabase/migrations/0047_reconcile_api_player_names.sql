...
begin
  ...
  with candidates as (
      select legacy.id as legacy_id,api.full_name,api.position,api.club,api.competition,api.photo_url,api.nationality,
      ...
      join public.players api on api.api_football_id is not null
        and lower(extensions.unaccent(api.full_name))=lower(extensions.unaccent(legacy.full_name))
