...
begin
  ...
  select array_agg(p.id order by coalesce(p.draft_rank, 2147483647), p.id)
  into v_missing_ids
  ...
  where p.active
    ...
    and not exists (
      select 1
      from jsonb_array_elements_text(p_api_ids) x(api_id)
      where x.api_id::bigint = p.api_football_id
    );

  update public.players
  set draft_rank = null
  where active and api_football_id is not null and draft_rank is not null;
  ...
  with missing as (
    select id, ordinality::integer rn
    from unnest(coalesce(v_missing_ids, array[]::bigint[])) with ordinality as m(id, ordinality)
  )
  update public.players p
