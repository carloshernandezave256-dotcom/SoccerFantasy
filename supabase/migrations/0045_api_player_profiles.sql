alter table public.players add column if not exists api_football_id bigint;
...
create unique index if not exists players_api_football_id_key on public.players(api_football_id) where api_football_id is not null;
...
declare
  ...
  v_id bigint;
begin
  if not exists(select 1 from public.leagues where commissioner_id=(select auth.uid())) then
...
grant execute on function public.sync_api_football_players(jsonb) to authenticated;
