...
Native PostgreSQL concurrency passed in GitHub Actions run 35309934948 on commit 2e09bcc. Local validation now includes 141 app tests, 21 market and 30 scoring database checks. Release still requires current provider payloads to satisfy the strict squad policy, production fixture membership review, and a decision on the missed waiver-window policy. The current league's Week 4 contains an inactive locked Reijnders row with missing evidence; do not fabricate a zero. Applying the migration and deploying must be coordinated. No production migration, bulk rescore, merge, or deployment has been performed.
